...
describe('Function recordParameters()', function () {
  ...
  // executing...
  const records: ParameterRecord[] = recordParameters(signInFn, register);
  ...
  it(`should clear parameter register`, function () {
    const res = register.apply(emailIndex);
    assert.ok(!res.validators[Symbol.keyFor(requiredSymbol)]);
  })
});